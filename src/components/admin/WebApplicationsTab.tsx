...
function SortableTypeItem({ type, onEdit, onDelete }: { type: WebType; onEdit: () => void; onDelete: () => void }) {
...
function SortableExampleItem({ example, onEdit, onDelete }: { example: WebExample; onEdit: () => void; onDelete: () => void }) {
...
export function WebApplicationsTab() {
  const [webTypes, setWebTypes] = useState<WebType[]>([]);
  const [webExamples, setWebExamples] = useState<WebExample[]>([]);
  const [selectedType, setSelectedType] = useState<string | null>(null);
  const [editingType, setEditingType] = useState<WebType | null>(null);
  const [editingExample, setEditingExample] = useState<WebExample | null>(null);
  ...
  useEffect(() => {
    if (selectedType) {
      fetchWebExamples(selectedType);
    }
  }, [selectedType]);

  const fetchWebTypes = async () => {
    ...
    if (error) {
      toast.error("Failed to fetch web types");
      return;
    }
    setWebTypes(data || []);
  };

  const fetchWebExamples = async (typeId: string) => {
    const { data, error } = await supabase
      .from("web_examples")
      .select("*")
      .eq("web_type_id", typeId)
      .order("display_order", { ascending: true, nullsFirst: false });
  ...
  const handleSaveType = async () => {
    ...
    if (editingType) {
      ...
      const { error } = await supabase
        .from("web_types")
        .insert({ name: typeName, display_order: webTypes.length });
  ...
  const handleDeleteType = async (id: string) => {
    const { error } = await supabase.from("web_types").delete().eq("id", id);
  ...
  const handleSaveExample = async () => {
    ...
    if (editingExample) {
      ...
      const { error } = await supabase
        ...
        .insert({ 
          name: exampleName, 
          web_type_id: selectedType,
          display_order: webExamples.length 
        });
  ...
  const handleDeleteExample = async (id: string) => {
    const { error } = await supabase.from("web_examples").delete().eq("id", id);
  ...
  const handleDragEndTypes = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = webTypes.findIndex((t) => t.id === active.id);
      const newIndex = webTypes.findIndex((t) => t.id === over.id);
      const newOrder = arrayMove(webTypes, oldIndex, newIndex);
      setWebTypes(newOrder);

      // Update display_order in database
      const updates = newOrder.map((type, index) => 
        supabase.from("web_types").update({ display_order: index }).eq("id", type.id)
      );
  ...
  const handleDragEndExamples = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = webExamples.findIndex((e) => e.id === active.id);
      const newIndex = webExamples.findIndex((e) => e.id === over.id);
      const newOrder = arrayMove(webExamples, oldIndex, newIndex);
      setWebExamples(newOrder);

      // Update display_order in database
      const updates = newOrder.map((example, index) => 
        supabase.from("web_examples").update({ display_order: index }).eq("id", example.id)
      );
      await Promise.all(updates);
      toast.success("Order updated");
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Web Applications Management</h2>
        <Dialog open={typeDialogOpen} onOpenChange={setTypeDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingType ? "Edit" : "Add"} Web Type</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <Input
                placeholder="Web Type Name"
                value={typeName}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-4">
          <h3 className="text-lg font-semibold mb-4">Web Types</h3>
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEndTypes}>
            <SortableContext items={webTypes.map(t => t.id)} strategy={verticalListSortingStrategy}>
        ...
        <Card className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold">
              Examples {selectedType && `(${webTypes.find(t => t.id === selectedType)?.name})`}
            </h3>
            {selectedType && (
          ...
          {selectedType ? (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEndExamples}>
              <SortableContext items={webExamples.map(e => e.id)} strategy={verticalListSortingStrategy}>
                <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-center py-8">Select a web type to manage examples</p>
          )}
        </Card>
      </div>
    </div>
